...
export default function BlurTextAnimation({
    ...
    words,
    className = "",
    fontSize = "text-xl", // Adjusted default for chat
    ...
    useEffect(() => {
        const startAnimation = () => {
            ...
            // We don't want to loop in the chat, just show once.
            // So I removed the reset loop for the chat use-case if it's not desired.
            // But for "simulating AI generation" maybe we want it to just appear.
            // The user code had a loop. I will keep the loop logic but maybe we can control it.
            // For now, I'll stick to the user's provided code structure but maybe remove the infinite loop to make it usable in chat.

            animationTimeoutRef.current = setTimeout(() => {
                // setIsAnimating(false); // don't fade out
        ...
        <div className={` ${className}`}>
            <p className={`${textColor} ${fontSize} ${fontFamily} font-light leading-relaxed tracking-wide`}>
                {textWords.map((word, index) => (
                    <span
                        key={index}
                        className={`inline-block transition-all duration-300 ${isAnimating ? 'opacity-100' : 'opacity-0'}`}
                        style={{
                            transitionDuration: `${word.duration}s`,
                            transitionDelay: `${word.delay}s`, // Add delay to stagger appearance
