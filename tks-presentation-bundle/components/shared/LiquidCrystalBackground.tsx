...
void main() {
  ...
  // Microsoft Blue/Cyan tones: 
  // We want primarily blues and cyans. 
  // Original pha was: 0.5 + 0.5 * cos(u_time * 0.5 + uv.xyx + vec3(0,1,2));
  // Let's tune it to be more cooling/liquid blue.
  vec3 pha = 0.5 + 0.5 * cos(u_time * 0.2 + uv.xyx + vec3(0, 2, 4)); 
  
  // Mix in a strong blue base
  vec3 blueBase = vec3(0.0, 0.6, 1.0);
  vec3 col = clamp(base * pha * blueBase * 4.0, 0.0, 1.0);
  
  // Add a soft glow core 
  if (d < 0.0) {
      col += vec3(0.0, 0.2, 0.5) * 0.5;
  }
